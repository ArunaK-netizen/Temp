...
COURSES_FILE = args.courses if 'args' in globals() else args.courses
RAS_FILE = args.ras if 'args' in globals() else args.ras
SLOT_MAP_FILE = args.slotmap if 'args' in globals() else args.slotmap
OUTPUT_FILE = args.output if 'args' in globals() else args.output

# If argparse didn't run (imported), try environment variables
if not COURSES_FILE:
    COURSES_FILE = os.environ.get('COURSES_FILE', DEFAULT_COURSES_FILE)
if not RAS_FILE:
    RAS_FILE = os.environ.get('RAS_FILE', DEFAULT_RAS_FILE)
if not SLOT_MAP_FILE:
    SLOT_MAP_FILE = os.environ.get('SLOT_MAP_FILE', DEFAULT_SLOT_MAP_FILE)
if not OUTPUT_FILE:
...
# 1. Each lab assigned at most once
for lab in labs:
...
# 4. No overlap same slot for same RA
for ra in ras:
    slot_groups = {}
    for lab in labs:
        slot_key = lab['slot']
...
for ra in ras:
    ...
    for course in unique_courses:
        ...
        prob += lpSum([x[(ra['id'], lab['id'])] for lab in course_labs]) <= len(course_labs)*course_used[(ra['id'], course)], f"ra_{ra['id']}_course_{course}_link"
    ...
    for i, lab1 in enumerate(labs):
        for j, lab2 in enumerate(labs):
            if i < j and are_back_to_back(lab1['slot'], lab2['slot']):
                ...
                prob += btb_var >= x[(ra['id'], lab1['id'])] + x[(ra['id'], lab2['id'])] - 1, f"btb_{ra['id']}_{lab1['id']}_{lab2['id']}_link"
